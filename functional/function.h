...
struct _copyable_function {
  ...
  struct empty_object : lvalue_callable {
    void copy_into_(void* location) const override { ::new (location) Self; }

    void move_into_(void* location) noexcept override { ::new (location) Self; }
  };
...
class function<S, R(Args...)> {
  ...
  template <class F>
  using target_object_for = typename copyable_function::template target_object<
  ...
  template <auto f, class T>
  using bound_target_object_for = typename copyable_function::
      template bound_target_object<f, std::unwrap_ref_decay_t<T>>;
  ...
  alignas(
      typical_target_object) std::byte storage_[sizeof(typical_target_object)];
 ...
 public:
  ...
  R operator()(Args... args) const
    requires(!signature::is_variadic)
  {
...
template <class S>
using _strip_noexcept_t = typename _strip_noexcept<S>::type;
