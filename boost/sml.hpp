...
struct is_empty : aux::integral_constant<bool,
                                         sizeof(is_empty_base<T, none_type>) ==
                                             sizeof(none_type)> {};
template <class>
...
struct pool_type_impl<T&,
                      aux::enable_if_t<aux::is_constructible<T>::value &&
...
struct get_all_events_impl<sm<TSrc>, sm<TDst>, TEvent> {
  using type = aux::join_t<get_event<TEvent>,
...
using get_sub_internal_events = aux::join_t<
    typename get_sub_internal_events_impl<typename Ts::src_state,
                                          typename Ts::event>::type...,
    typename get_sub_internal_events_impl<typename Ts::dst_state,
                                          typename Ts::event>::type...>;
template <class... Ts>
...
using get_initial_states = aux::join_t<
    typename aux::conditional<Ts::initial,
                              aux::type_list<typename Ts::src_state>,
                              aux::type_list<>>::type...>;
template <class... Ts>
using get_history_states = aux::join_t<
    typename aux::conditional<!Ts::history && Ts::initial,
                              aux::type_list<typename Ts::src_state>,
                              aux::type_list<>>::type...>;
template <class>
...
using get_non_empty_t =
    aux::join_t<typename aux::conditional<aux::is_empty<Ts>::value,
                                          aux::type_list<>,
...
struct transitions<aux::false_type> {
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute(const TEvent&,
                      SM&,
                      TDeps&,
                      TSubs&,
                      typename SM::state_t&) {
...
struct transitions_sub<sm<TSM>, T, Ts...> {
  ...
  template <class TEvent, class SM, class TDeps, class TSubs>
  static bool execute_impl(const TEvent& event,
                           SM& sm,
                           TDeps& deps,
                           TSubs& subs,
                           typename SM::state_t& current_state) {
...
struct transitions_sub<sm<TSM>> {
  ...
  template <class, class SM, class TDeps, class TSubs>
  static bool execute(const anonymous&,
                      SM&,
                      TDeps&,
                      TSubs&,
                      typename SM::state_t&) {
...
struct conditional_mapping<true,
                           aux::type<T1, aux::inherit<Rs...>>,
                           T2,
                           Ts...> {
  using type = unique_mappings_impl<
      aux::type<aux::inherit<T1>, extend_mapping_t<T2, Rs...>>,
      Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct conditional_mapping<false,
                           aux::type<T1, aux::inherit<Rs...>>,
                           T2,
                           Ts...> {
  using type = unique_mappings_impl<
...
void log_process_event(const aux::type<TLogger>&,
                       TDeps& deps,
                       const TEvent& event) {
...
void log_state_change(const aux::type<no_policy>&,
                      TDeps&,
                      const TSrcState&,
                      const TDstState&) {}
template <class SM,
...
void log_action(const aux::type<no_policy>&,
                TDeps&,
                const TAction&,
                const TEvent&) {}
...
void log_guard(const aux::type<no_policy>&,
               TDeps&,
               const TGuard&,
               const TEvent&,
               bool) {}
...
struct sm_impl : aux::conditional_t<aux::is_empty<typename TSM::sm>::value,
  ...
  template <class TEvent,
            ...
            __BOOST_SML_REQUIRES(
                aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value &&
                !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
  bool process_internal_event(const TEvent& event,
                              TDeps& deps,
                              TSubs& subs,
                              state_t& current_state) {
  ...
  bool process_event_except_imp(const TEvent& event,
                                TDeps& deps,
                                TSubs& subs,
                                aux::false_type) {
  ...
  bool process_exception(TDeps& deps,
                         TSubs& subs,
...
decltype(auto) get_arg(const aux::type<TEvent>&,
                       const TEvent& event,
                       TSM&,
                       TDeps&) {
  return event;
...
decltype(auto) get_arg(const aux::type<back::defer<TEvents...>>&,
                       const TEvent,
                       TSM& sm,
                       TDeps&) {
...
struct call<TEvent, aux::type_list<>, TLogger> {
  ...
  static auto execute_impl(const aux::type<bool>&,
                           T object,
                           const TEvent& event,
                           TDeps& deps) {
  ...
  template <class TSM, class T, class TDeps>
  static auto execute_impl(const aux::type<void>&,
                           T object,
                           const TEvent& event,
                           TDeps& deps) {
...
class seq_ : operator_base {
 ...
 private:
  ...
  void for_all(const aux::index_sequence<Ns...>&,
               const TEvent& event,
               TSM& sm,
               TDeps& deps,
               TSubs& subs) {
...
class and_ : operator_base {
 ...
 private:
  template <int... Ns, class TEvent, class TSM, class TDeps, class TSubs>
  auto for_all(const aux::index_sequence<Ns...>&,
               const TEvent& event,
               TSM& sm,
               TDeps& deps,
               TSubs& subs) {
...
struct transition<state<S2>, G, A> : transition<state<internal>,
                                                state<S2>,
                                                front::event<back::anonymous>,
                                                G,
                                                A> {
  using transition<state<internal>,
                   state<S2>,
                   front::event<back::anonymous>,
                   G,
                   A>::g;
...
struct transition_sg<state<S2>, G> : transition<state<internal>,
                                                state<S2>,
                                                front::event<back::anonymous>,
                                                G,
                                                none> {
...
struct transition_sa<state<S2>, A> : transition<state<internal>,
                                                state<S2>,
                                                front::event<back::anonymous>,
                                                always,
                                                A> {
...
void update_composite_states(TSubs& subs,
                             aux::true_type,
