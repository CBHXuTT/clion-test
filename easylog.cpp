...
inline void log(spdlog::level::level_enum level, source_location location,
    ...
    switch (level) {
        case spdlog::level::trace:
            spdlog::trace("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                          fmt::format(fmt, std::forward<Args>(args)...));
            break;
        case spdlog::level::debug:
            spdlog::debug("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                          fmt::format(fmt, std::forward<Args>(args)...));
            break;
        case spdlog::level::info:
            spdlog::info("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                         fmt::format(fmt, std::forward<Args>(args)...));
            break;
        case spdlog::level::warn:
            spdlog::warn("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                         fmt::format(fmt, std::forward<Args>(args)...));
            break;
        case spdlog::level::err:
            spdlog::error("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                          fmt::format(fmt, std::forward<Args>(args)...));
            break;
        case spdlog::level::critical:
            spdlog::critical("[{}] {}:{}: {}", location.module_name(), p.filename().string(), location.line(),
                             fmt::format(fmt, std::forward<Args>(args)...));
            break;
...
LogMessage::LogMessage(int level, source_location location) : level_(level),
                                                              location_(location) {}
