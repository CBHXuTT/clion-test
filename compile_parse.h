...
// https://godbolt.org/z/bszMKndsq

#ifndef CLION_COMPILE_PARSE_H
#define CLION_COMPILE_PARSE_H
  ...
  constexpr auto token() -> Token {
    ...
    if (i < s.size()) {
      ...
      if (is_digit(s[i])) {
        ...
        return Token{TokenType::Number, from, i, 0};
      }
      ...
      switch (s[i]) {
        ...
        case '>':
          ...
          if (i < s.size() && s[i] == '=') {
            i += 1;
            return Token{TokenType::GreaterEqual, from, i};
          }
          return Token{TokenType::GreaterThan, from, i};
        case '<':
          i += 1;
          if (i < s.size() && s[i] == '=') {
            i += 1;
            return Token{TokenType::LessEqual, from, i};
          }
          return Token{TokenType::LessThan, from, i};
        case '!':
          i += 1;
        ...
        case '{':
          i += 1;
          return Token{TokenType::LeftCurly, from, i};
        case '}':
          i += 1;
          return Token{TokenType::RightCurly, from, i};
        case ',':
          i += 1;
          return Token{TokenType::Comma, from, i};
        case ':':
          i += 1;
          return Token{TokenType::Colon, from, i};
        case ';':
          i += 1;
          return Token{TokenType::Semicolon, from, i};
        case '$':
          i += 1;
          return Token{TokenType::Dollar, from, i};
        default:
          i += 1;
